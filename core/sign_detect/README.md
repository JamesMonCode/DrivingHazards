...
+ [`Training`](/Training/): Scripts and instructions on training your YOLOv3 model
...
To speed up training, it is recommended to use a **GPU with CUDA** support. For example on [AWS](/Training/AWS/) you can use a `p2.xlarge` instance (Tesla K80 GPU with 12GB memory). Inference is very fast even on a CPU with approximately ~2 images per second. 
